 ...
 *
 * Uses MailChannels API (free for Cloudflare Workers) to send emails.
 * Requires DNS TXT record on the sender domain:
 *   _mailchannels.<domain> TXT "v=mc1 ..."
 *
 * Alternatively, set EMAIL_PROVIDER=resend and provide RESEND_API_KEY.
 */
...
export async function sendWelcomeEmail(
  ...
  const htmlBody =
    ...
    `<p>本サービスは、SBI証券の外国株式IPOページにおいて、<strong>「購入申込する」ボタンが押せるようになった際</strong>に、ご登録のメールアドレスへ自動的にお知らせするサービスです。</p>` +
    ...
    `<p style="margin:0;font-size:14px;">📌 登録解除は以下のURLから：<br><a href="${unsubscribeUrl}" style="color:#666;word-break:break-all;">${unsubscribeUrl}</a></p></div>` +
    `</div>` +
    `<div style="text-align:center;padding:20px;color:#999;font-size:12px;">` +
    `<p>※ 本メールは自動送信されています。</p><p>${config.siteTitle} | <a href="${config.siteUrl}" style="color:#999;">${config.siteUrl}</a></p></div>` +
    `</body></html>`;
...
export async function sendIPOAvailableEmail(
  ...
  const sbiUrl =
    ...
    "&_PageID=WPLETmgR001Mdtl30&_ActionID=DefaultAID" +
    "&_DataStoreID=DSWPLETmgR001Control&OutSide=on&getFlg=on" +
    "&burl=search_foreign&cat1=foreign&cat2=ipo&dir=ipo" +
    "&file=foreign_ipo_260527.html";
  ...
  const htmlBody =
    ...
    `<li>本通知は自動チェックによるものです。実際の申込可否はSBI証券のページでご確認ください。</li>` +
    `<li>申込期間は限られています。お早めにご確認ください。</li></ul></div>` +
    `<p style="font-size:13px;color:#999;margin-top:30px;">登録解除：<a href="${unsubscribeUrl}" style="color:#999;">${unsubscribeUrl}</a></p>` +
    `</div>` +
    `<div style="text-align:center;padding:20px;color:#999;font-size:12px;"><p>${config.siteTitle}</p></div>` +
    `</body></html>`;
...
// =============================================================================
// Internal: Email sending via MailChannels
// =============================================================================
...
async function sendEmail(
  ...
  try {
    ...
    // MailChannels returns 202 on success, anything else is an error
    return {
      success: false,
      error: `MailChannels API error ${response.status}: ${errorBody.substring(0, 300)}`,
    };
  } catch (error: unknown) {
